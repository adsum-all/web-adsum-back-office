...
// Rich-text editor for the activity description: bold, italic, underline, lists and
// a heading, on several lines. Emits an HTML string; the server sanitises it through
// an allowlist before storing and it is rendered back through the same allowlist.
// Shared shape with the collaboration app (adsum-collaboration/.../RichEditor.tsx).
...
export function RichEditor({ value, onChange, disabled }: Props): JSX.Element {
  ...
  return (
    <div style={{ border: "1px solid var(--adsum-line, #d7dbe3)", borderRadius: 10, overflow: "hidden" }}>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, padding: 6, borderBottom: "1px solid var(--adsum-line, #e2e2e2)", background: "var(--adsum-bg-soft, #f7f8fa)" }}>
        {TOOLS.map((t) => (
          <button
            key={t.label}
            type="button"
